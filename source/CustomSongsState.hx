...
class CustomSongsState extends MusicBeatState
{
	...
	override function create()
	{
		...
		{
			var songInfo = Song.loadFromJson(customSongList[i], customSongList[i]);
			var author = songInfo.author != "" ? songInfo.author : "No Author";
			songs.push(new SongMetadata(customSongList[i], -1, songInfo.player2, author));
		}

		#if desktop
		DiscordClient.changePresence("Browsing Custom Songs", null);
		#end
		...
		authorText = new FlxText(scoreText.x, diffText.y + 55, 0, "", 15);
		authorText.font = scoreText.font;
	...
	override function update(elapsed:Float)
	{
		...
		authorText.text = "Author: " + songs[curSelected].songAuthor;
	...
	function changeSelection(change:Int = 0)
	{
		...
		// lerpScore = Highscore.getScore(songs[curSelected].songName, curDifficulty);
